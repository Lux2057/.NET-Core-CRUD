...
public class TransactionScopeTests : EfUnitOfWorkTest
{
    ...
    public TransactionScopeTests(IUnitOfWork unitOfWork, IRepository repository, TestDbContext context)
            : base(unitOfWork, repository, context) { }
    ...
    public async Task Should_rollback_changes_in_a_transaction()
    {
        ...
        var entities = await this.Repository.Read<TestEntity>().ToArrayAsync();
        ...
        entities = await this.Repository.Read<TestEntity>().ToArrayAsync();
